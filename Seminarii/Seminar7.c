...
struct Galerie { // galerie de arta

	int cod; // pt hash-code
	char* nume;
	float pretIntrare;
};
...
struct nod { // lista simplu inlantuita
	Galerie info;
	nod* next;
};
...
void inserareGalerieInTabela(HashTable hash, Galerie galerie) {
	if (hash.dim > 0) {
		...
		if (hash.vector[pozitie] != NULL) { // exista element, nu pot sa pun pe acceasi pozitie, deci inseram in lista(situatie de coliziune)
